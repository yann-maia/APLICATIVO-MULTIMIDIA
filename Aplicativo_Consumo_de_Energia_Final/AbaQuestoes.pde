...
void drawQuestionsPlaying(){
  ...
  stroke(180);
  strokeWeight(1);
  rect(caixaX, caixaY, caixaW, caixaH, 20);
  ...
  textAlign(CENTER);
  textSize(22);
  text("Pontua\u00e7\u00e3o: " + pontos, width/2, height * 0.93);
}
...
void drawQuestionsFinished(){
  ...
  if(pontos == 10){
    ...
    text("Excelente desempenho!", width/2, height * 0.52);

  } else if(pontos >= 7){

    text("Muito bom!", width/2, height * 0.52);

  } else {

    text("Continue praticando!", width/2, height * 0.52);
  }
...
void drawQuizAlternative(float x, float y, float w, float h, String label){
...
void desenharQuestaoBandeiras(){
  ...
  fill(0);
  textAlign(CENTER);
  text("BANDEIRAS", bandeiraX + caixaW/2, inicioY - 35);
  text("DESCRI\u00c7\u00d5ES", descricaoX + caixaW/2, inicioY - 35);
  ...
  for(int i = 0; i < bandeiras.length; i++){
    ...
    stroke(150);
    strokeWeight(1);
    rect(bandeiraX, inicioY + i * espaco, caixaW, caixaH, 15);

    fill(0);
    textAlign(CENTER, CENTER);
    text(bandeiras[i], bandeiraX + caixaW/2, inicioY + i * espaco + caixaH/2);

    fill(255);
    stroke(150);
    rect(descricaoX, inicioY + i * espaco, caixaW, caixaH, 15);

    fill(0);
    text(descricoesBandeiras[i], descricaoX + caixaW/2, inicioY + i * espaco + caixaH/2);
